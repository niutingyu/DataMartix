...
#define kGreen  RGBA(112, 175, 245, 1)
#define kRed    RGBA(112, 175, 245, 1)
#define kGray   RGBA(112, 175, 245, 1)
#define kLineGray RGBA(112, 175, 245, 1)
...
- (void)singleViewWithFrame:(CGRect)frame titleStr:(NSString *)title detailStr:(NSString *)detail model:(kProgressType)type isFirst:(BOOL)isFirst isEnd:(BOOL)isEnd isHorizontal:(BOOL)isHorizontal style:(kProgressStyle)style{
    ...
    line.backgroundColor = RGBA(112, 175, 245, 1);
    
    UIImageView *backImageView = [[UIImageView alloc] init];
   // [backImageView.image imageChangeColor:RGBA(112, 175, 245, 1)];
    switch (type) {
            
        case kProgressTypeFirst:
            iconView.image = [UIImage imageNamed:@"luyin-normal"];
        ...
        case kProgressTypePassNormal:
            iconView.image = [UIImage imageNamed:@"luyin-small"];
        ...
        case kProgressTypeCurrentNormal:
            ...
            iconView.frame =CGRectMake(0, 0, 20, 20);
            [iconView.image imageChangeColor:RGBA(112, 175, 245, 1)];
            ...
            if (style==kProgressStyleSpecial) {
                iconView.frame = CGRectMake(0, 0, 20, 20);
                if (isHorizontal) {
                    detalLabel.textColor = RGBA(112, 175, 245, 1);
                }
            }
            break;
        ...
        case kProgressTypeWarning:
            ...
            titleLabel.textColor = RGBA(110, 175, 248, 1);
            detalLabel.textColor = kGray;
            break;
    ...
    if (isHorizontal) {
        ...
        titleLabel.frame = CGRectMake((frame.size.width - titleSize.width) * 0.5, 28, titleSize.width, titleSize.height+20);
...
-(void)tapMethod:(UITapGestureRecognizer*)tap{
    ...
    debugLog(@"-- %ld ",iconView.tag);
    if (self.progressTap) {
        self.progressTap(iconView.tag);
    }
}
